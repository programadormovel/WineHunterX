...
namespace WineHunterX.Droid
{
	...
	{
		internal static event EventHandler<WineHunterX.View.Util.XViewEventArgs> MediaCroped;
		...
		private void OnSaveClicked()
		{
			...
			{
				var b = croppedImage;
				BackgroundJob.StartBackgroundJob(this, null, "Salvando Imagem", () => SaveOutput(b), _mHandler);
			}
		...
		private void SaveOutput(Bitmap croppedImage)
		{
			...
			{
				Log.Error(GetType().Name, "caminho da imagem não definido");
			}
			croppedImage.Recycle();
			Finish();
		}
		...
		private static void ShowStorageToastWithRemain(Activity activity, int remaining)
		{
			...
			if (remaining == NoStorageError)
			{
				...
				noStorageText = state == Android.OS.Environment.MediaChecking ? "Preparando armazenamento" : "Memory Card não encontrado";
			}
			else if (remaining < 1)
			{
				noStorageText = "Espaço insuficiente";
			}
